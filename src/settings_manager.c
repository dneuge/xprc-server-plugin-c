...
error_t configure_settings_manager_from_storage(settings_manager_t *settings_manager) {
    ...
    if (is_first_initialization) {
        ...
        if (err != ERROR_NONE) {
            printf("[XPRC] settings manager failed to save settings to %s: %d\n", settings_manager->settings_filepath, err);
            failed_settings_save = true;
        }
    } else {
        err = load_settings_without_password(settings_manager->settings, settings_manager->settings_filepath);
        if (err != ERROR_NONE) {
            printf("[XPRC] settings manager failed to load settings from %s: %d\n", settings_manager->settings_filepath, err);
    ...
    if (should_regenerate_password) {
        ...
        if (err != ERROR_NONE) {
            printf("[XPRC] settings manager failed to save password to %s: %d\n", settings_manager->password_filepath, err);
            failed_password_save = true;
        }
    } else {
        err = load_password(settings_manager->settings, settings_manager->password_filepath);
        if (err != ERROR_NONE) {
            printf("[XPRC] settings manager failed to load password from %s: %d\n", settings_manager->password_filepath,
                   err);
