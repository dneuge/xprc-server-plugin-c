...
static void destroy_channels_subtable(channels_subtable_t *subtable) {
    // FIXME: call on_destruction callback for each channel
    free(subtable);
}

void destroy_channels_table(channels_table_t *table) {
    for (int i=0; i<CHANNELS_NUM_SUBTABLES; i++) {
        channels_subtable_t *subtable = table->subtables[i];
        if (subtable) {
            destroy_channels_subtable(subtable);
